...
{spine("聚合怎么切、模式对应哪条优惠/售后需求、单体模块化 vs 微服务怎么拍板——全是改代码用的，不背定义。",
       serves="B-F 结算履约 · B-R 售后",
       back="S2 → 本页 → S-MS-X / B-X")}
{plain("人话：DDD 不是画六边形好看。问三句就够——谁能改这张表？状态谁说了算？跨系统脏模型谁翻译？模式是「需求老变时少改炸点」的工具。")}
  <table>
...
{c4(
    "五拨人别抢同一张订单表改库存字段；退款时要找得到「当时优惠快照」。",
    "订单聚合持单头+行+支付意图+优惠快照；优惠上下文持规则；库存持预占；OMS 持履约单；售后持售后单+退款单。",
    "聚合=事务与不变性边界；跨上下文用 ID+事件，不 join 他库。",
    "任意单号能指出写库服务；禁止售后 UPDATE 原优惠规则行。",
    "峰值下同步双写最容易资损。",
)}
...
''')}
  <table>
    <thead><tr><th>上下文</th><th>写权威表（例）</th><th>别的上下文只能</th><th>今天代码怎么约束</th></tr></thead>
    <tbody>
      <tr><td>订单</td><td>orders, order_item, discount_snapshot</td><td>读快照 / 发事件</td><td>包 <code>order</code> 外禁止写 mapper</td></tr>
      <tr><td>优惠</td><td>promo_rule, coupon</td><td>RPC 试算返回 DTO</td><td>规则热更不影响历史快照</td></tr>
      <tr><td>库存</td><td>sku_stock, stock_reserve</td><td>收事件确认/释放</td><td>无订单库账号</td></tr>
      <tr><td>履约</td><td>fulfillment_order</td><td>收支付成功</td><td>Inbox 去重</td></tr>
      <tr><td>售后</td><td>after_sale, refund</td><td>读订单快照</td><td>回退分摊写售后侧表</td></tr>
    </tbody>
  </table>
{today("""<ul>
<li>新建包：<code>domain.order</code> / <code>domain.stock</code>…；跨包只依赖接口或事件 DTO。</li>
<li>下单事务：写订单+快照+outbox，<b>不</b>写库存表。</li>
<li>评审检查：有没有跨服务 join、有没有售后改历史规则。</li>
</ul>""")}
{qa("【实战】优惠常变要独立服务，下单又要同事务——怎么切？",
    ["试算同步短超时；落单把结果拷进 discount_snapshot；规则服务可独立部署。退款只读快照。",
     "规则周更。", "订单库存可变脚本。", "快照表+版本号。", "「可变规则外置，成交结果内聚。」"],
    "ddd-bc-q1")}
{reflect("ddd-bc-r1")}
</section>
"""
...
{qa("【实战】寄修与退货并行，状态机怎么切？",
    ["售后单类型分状态机，或并行子单；库存预占用事件。禁止一个枚举硬塞所有分支。挂 B-X 寄修案。",
...
{qa("【题】老板要一周拆 20 服务，如何用权衡表挡？",
    ["把维度表打分摊开：团队、对账、事务成本；给模块化里程碑。对齐的是资损边界不是个数。",
     "转型会议。", "硬拆共享库。", "ADR。", "「先边界后进程。」"],
    "ddd-arch-q1")}
{reflect("ddd-arch-r1")}
</section>
"""
    return hub + bc + acl + patterns + arch
